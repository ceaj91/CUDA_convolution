...
__global__ void conv_2D_constant_mem(float *in_data, float *out_data, int kernel_width, int data_width_x,int data_width_y,int channel_size)
{
...
int main()
{
   ...
   for (int ch = 0; ch < KERNEL_CHANNEL_SIZE; ch++)
   {
	...
	dim3 gridDim(ceil((float)OUTPUT_SIZE_X/(float)BLOCK_DIM_X),ceil((float)OUTPUT_SIZE_Y/(float)BLOCK_DIM_Y));

	//RUN CUDA KERNEL

	conv_2D_constant_mem<<< gridDim, blockDim >>>(d_input_array, d_output_array, KERNEL_SIZE_X,INPUT_SIZE_X,INPUT_SIZE_Y,INPUT_CHANNEL_SIZE);

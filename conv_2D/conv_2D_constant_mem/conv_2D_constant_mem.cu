...
__global__ void conv_2D_constant_mem( float *in_data, float *out_data, int kernel_width, int data_width)
{
	...
	for(int ki = 0; ki<kernel_width; ki++)
	{
		for(int kj = 0; kj<kernel_width; kj++)
		{
			...
			if(ni>=0 && ni < data_width && nj>=0 && nj<data_width)
				Pvalue += in_data[ni*data_width + nj] * d_kernel[ki*kernel_width + kj];    
		}
	}
...
//INPUT FOR SEQ PART
	...
	conv_2D_constant_mem<<< gridDim, blockDim >>>(d_input_array, d_output_array, KERNEL_SIZE_X,INPUT_SIZE_X);
